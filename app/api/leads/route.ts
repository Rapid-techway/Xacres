...
export async function POST(req: NextRequest) {
  try {
    const { landId, name, phone, budget, note } = await req.json();

    // 1. Basic validation and abuse protection
    if (!landId || !name || !phone) {
      return NextResponse.json({ error: 'Missing required fields: landId, name, phone' }, { status: 400 });
    }

    if (name.length > 100 || phone.length > 25 || (budget && budget.length > 100) || (note && note.length > 1000)) {
    ...
    const { data: land, error: landError } = await supabase
      .from('lands')
      .select('title, slug, district, village, area, price')
      .eq('id', landId)
      .maybeSingle();
    ...
    if (botToken && chatId) {
      try {
        ...
        const text = `🔔 <b>New Lead on Xacres!</b>\n\n` +
          ...
          `📏 <b>Size:</b> ${land.area} Acres\n\n` +
          `👤 <b>Name:</b> ${name}\n` +
          `📞 <b>Phone:</b> ${phone}\n` +
          `💰 <b>Budget:</b> ${budget || 'Not specified'}\n` +
          `📝 <b>Message:</b> ${note || 'None'}\n\n` +
