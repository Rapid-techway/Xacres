...
interface LandsClientPageProps {
  initialLands: Land[];
}

export default function LandsClientPage({ initialLands }: LandsClientPageProps) {
  const { resetFilters, location, minPrice, maxPrice, minSize, maxSize } = useFilterStore();
  const storeLands = useLandStore((state) => state.lands);
  const storeIsLoading = useLandStore((state) => state.isLoading);
  const [showButton, setShowButton] = useState(true);
  const footerSentinelRef = useRef<HTMLDivElement>(null);

  // Sync server-fetched lands to Zustand store for client-side updates if empty
  useEffect(() => {
    if (storeLands.length === 0 && initialLands.length > 0) {
      useLandStore.setState({ lands: initialLands, isLoading: false });
    }
  }, [initialLands, storeLands]);

  // Use store lands if loaded, otherwise fall back to server-fetched initialLands (fixes SSR & Hydration)
  const lands = storeLands.length > 0 ? storeLands : initialLands;
  const isLoading = storeLands.length > 0 ? storeIsLoading : false;
